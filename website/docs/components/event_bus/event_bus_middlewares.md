...
The on-error middleware emits an event when the wrapped function **throws**. When the wrapped function fails, the middleware resolves the event payload from the function's arguments and the caught error, dispatches the configured event, and then re-throws the original error so the failure still propagates to the caller. If the payload resolves to `undefined`, no event is dispatched and the original error is re-thrown as-is.
