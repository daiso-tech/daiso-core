 ...
 * from the file key extension (via MIME lookup) only when the metadata's
 * `contentType` is `null`; otherwise the existing content type is preserved.
 * When the extension is unknown, the content type falls back to
 * `application/octet-stream`, the most generic MIME type. When the underlying
 * adapter returns `null` (the file does not exist), the plugin passes `null`
 * through.
 *
